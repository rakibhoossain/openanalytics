...
export function Sidebar() {
  ...
  const { data: projects = [] } = useQuery({
    ...trpc.project.list.queryOptions({
      organizationId: activeTenantId || null,
    }),
  ...
  const { data: dashboards = [] } = useQuery({
    ...trpc.dashboard.list.queryOptions({
      projectId: activeShopId || '',
    }),
    enabled: !!activeShopId,
  });

  if (activeShopId && activeTenantId) {
    return (
  ...
  return (
    <SidebarContainer organizations={organizations} projects={projects}>
      ...
      <SidebarOrganizationMenu
        organization={organizations.find((o) => o.id === activeTenantId)!}
      />
    </SidebarContainer>
  );
}
...
export function SidebarContainer({
  ...
  const { projectId, shopId } = useAppParams();

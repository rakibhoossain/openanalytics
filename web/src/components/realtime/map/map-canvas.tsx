...
export const MapCanvas = memo(function MapCanvas({
  ...
  const { center, initialZoom } = useMemo(() => {
    ...
    const hull = getOuterMarkers(safeMarkers);
    const center =
      hull.length < 2
        ? getAverageCenter(safeMarkers)
        : calculateGeographicMidpoint(hull);

    const boundingBox = getBoundingBox(hull.length > 0 ? hull : safeMarkers);
    ...
    const initialZoom = safeMarkers.length > 0 ? autoZoom : 1;

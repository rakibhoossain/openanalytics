...
export function OverviewWidgetTablePages({
  ...
  return (
    <OverviewWidgetTable
      className={className}
      data={data ?? []}
      keyExtractor={(item) => (item.path || (item as any).name || 'page') + (item.origin || '')}
      getColumnPercentage={(item) => (maxSessions > 0 ? item.sessions / maxSessions : 0)}
      columns={[
        {
          name: 'Path',
          width: 'w-full',
          ...
          render(item) {
            const path = item.path || (item as any).name || '';

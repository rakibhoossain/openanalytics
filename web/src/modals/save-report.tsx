...
export default function SaveReport({
  ...
  const activeProjectId = shopId || projectId || '';
  const activeOrgId = tenantId || organizationId || '';
  const searchParams = useSearch({
    strict: false,
  }) as { dashboardId?: string } | undefined;
  ...
  const save = useMutation(
    trpc.report.create.mutationOptions({
      ...
      onSuccess(res) {
        ...
        const goToReport = () => {
          router.navigate({
            to: '/$tenantId/$shopId/reports/$reportId' as any,
            params: {
              tenantId: activeOrgId,
              shopId: activeProjectId,
              reportId: res.id,
            } as any,
...
function SelectDashboard({
  ...
  const safeDashboards = Array.isArray(dashboardQuery.data) ? dashboardQuery.data : [];
  const selectedDashboard = safeDashboards.find((d: any) => d.id === value);
    ...
    <div className="space-y-3">
      ...
      {!isCreatingNew ? (
        <div className="row gap-2 flex-wrap">
          {safeDashboards.map((dashboard: any) => (
            <Button
              type="button"
              key={dashboard.id}

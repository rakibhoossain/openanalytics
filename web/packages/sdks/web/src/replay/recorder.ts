...
export function startReplayRecorder(
  ...
  function flush(isFullSnapshot: boolean): void {
    ...
    if (payloadBytes > maxPayloadBytes) {
      ...
      // Single event exceeds limit — drop to avoid server rejection
      buffer = [];
      return;
    }
    ...
    try {
      ...
      console.error('[OpenAnalytics.Replay] sendChunk failed', err);
      throw err;
    }
  }
  ...
  const stopFn = record({
    ...
    maskTextSelector: maskAllText ? '*' : '[data-oa-mask]',

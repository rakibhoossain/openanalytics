...
function Chart({
  ...
  const flatSpikes = useMemo(
    () =>
      Array.isArray(referrerSpikes.data)
        ? referrerSpikes.data.flatMap((c) => c?.spikes || [])
        : null,
    [referrerSpikes.data]
  );
  ...
  return (
    ...
    >
      ...
      <OPReferences items={Array.isArray(references.data) ? references.data : []} />
      <OPReferrerSpikes
        items={Array.isArray(referrerSpikes.data) ? referrerSpikes.data : []}
